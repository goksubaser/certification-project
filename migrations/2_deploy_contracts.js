...
module.exports = async function (deployer) {
    ...
    let path = 'C:\\Users\\asus\\Desktop\\Dersler\\CMPE492\\certification-project-react-github\\src\\env.json'
    fs.writeFile(path, "{\"courseAddress\": \""+Course.address+"\",\"departmentAddress\": \""+Department.address+"\",\"diplomaAddress\": \""+Diploma.address+"\",\"facultyAddress\": \""+Faculty.address+"\",\"requestAddress\": \""+Request.address+"\",\"rolesAddress\": \""+Roles.address+"\"}", err => {
        if (err) {
            console.error(err)
            return
        }
